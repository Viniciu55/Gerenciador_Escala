...
function MemberPickerDialog({ open, onClose, members, role, dates, onSelect, onRemove, currentAssignments }: any) {
      ...
      <DialogContent className="max-w-sm max-h-[80vh] flex flex-col p-6">
        ...
        <div className="flex-1 overflow-y-auto -mx-2 px-2 space-y-1 mt-4">
          {currentAssignments && currentAssignments.length > 0 && (
            <>
              <div className="text-xs font-semibold text-muted-foreground px-1 py-1.5">Escalados:</div>
              {currentAssignments.map((assignment: any) => (
                <button key={assignment.id} onClick={() => onRemove(assignment.id)} className="flex items-center gap-3 w-full rounded-lg px-3 py-2.5 text-sm transition-colors hover:bg-destructive/10 text-destructive mb-1">
                  <span className="inline-flex items-center justify-center h-7 w-7 rounded-full bg-destructive/10"><Minus className="h-3.5 w-3.5" /></span><span className="font-medium">{assignment.member_name}</span>
                </button>
              ))}
              <div className="h-px bg-border my-2" />
              <div className="text-xs font-semibold text-muted-foreground px-1 py-1.5">Adicionar membro:</div>
            </>
          )}
          {sorted.map((member) => {
            const style = member.status ? STATUS_STYLES[member.status as ScheduleStatus] : null
            const isSelected = currentAssignments?.some((a: any) => a.member_email === member.email)
            return (
...
export function ScheduleBuilder({ scheduleType, onBack, showMergedCells = false }: { scheduleType: ScheduleType, onBack: () => void, showMergedCells?: boolean }) {
  ...
  const handleAssign = async (member: MemberAvailability) => {
    ...
    // Adiciona o membro aos assignments (permite múltiplos)
    setBuiltEntries(prev => {
      const newEntries = [...prev]
      dates.forEach(date => {
        // Verifica se o membro já está escalado para essa data/role
        const alreadyAssigned = newEntries.some(e => 
          e.schedule_date === date && 
          e.role === role.key && 
          e.member_email === member.email
        )
        if (!alreadyAssigned) {
          newEntries.push({
    ...
    try {
      const { insertBuiltScheduleEntry } = await import("@/lib/schedule-builder-api")
      await Promise.all(dates.map(d => insertBuiltScheduleEntry(scheduleType, d, role.key, member.name, member.email)))
    } catch { loadData() }
  }
    ...
    <div className="flex min-h-dvh flex-col bg-background">
                ...
                {getRolesForScheduleType(scheduleType).map(role => (
                  <tr key={role.key} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    {groupedColumns.map(col => {
                      ...
                      const assignments = builtEntries.filter(e => e.schedule_date === dates[0] && e.role === role.key)
                      return (
                        <td key={col.colKey} className="px-2 py-2 text-center min-w-[120px]">
                          <TooltipProvider delayDuration={300}><Tooltip><TooltipTrigger asChild>
                                <button onClick={async () => {
                                  ...
                                  className={`w-full rounded-lg border px-2 py-2 text-xs transition-all hover:ring-2 hover:ring-primary/30 active:scale-95 ${assignments.length > 0 ? "bg-primary/5 border-primary/20 text-foreground" : "bg-muted/30 border-dashed border-muted-foreground/20 text-muted-foreground"}`}
                                >
                                  {assignments.length > 0 ? (
                                    <div className="flex flex-col gap-1">
                                      {assignments.map((a) => (
                                        <div key={a.id} className="flex items-center justify-between gap-1 bg-background rounded px-1.5 py-0.5 text-[10px]">
                              ...
                              </TooltipTrigger><TooltipContent><p className="text-xs">{assignments.length > 0 ? `${assignments.map(a => a.member_name).join(", ")}` : `Escalar ${role.label}`}</p></TooltipContent></Tooltip></TooltipProvider>
                        </td>
                      )
                    })}
                  </tr>
      ...
      {pickerOpen && (
        <MemberPickerDialog
          ...
          onRemove={async (entryId: string) => {
            const { removeBuiltScheduleEntryById } = await import("@/lib/schedule-builder-api")
            setBuiltEntries(p => p.filter(e => e.id !== entryId))
            await removeBuiltScheduleEntryById(entryId)
          }}
          currentAssignments={builtEntries.filter(e => e.schedule_date === pickerOpen.dates[0] && e.role === pickerOpen.role.key)}
        />
      )}
    </div>
  )
