...
import { Home, Music, Headphones, MonitorPlay, ChevronRight, Lock } from "lucide-react"
import Link from "next/link"

const SCHEDULE_ICONS: Record<ScheduleType, React.ReactNode> = {
  louvor: <Music className="h-6 w-6" />,
  sonoplastia: <Headphones className="h-6 w-6" />,
  midia: <MonitorPlay className="h-6 w-6" />,
}
        ...
        <div className="space-y-3">
          {(Object.keys(SCHEDULE_CONFIG) as ScheduleType[]).map((type) => {
            ...
            return (
              ...
              >
                <span
                  className={`inline-flex items-center justify-center h-12 w-12 rounded-lg ${colors.bg} ${colors.text}`}
                >
                  {SCHEDULE_ICONS[type]}
                </span>
